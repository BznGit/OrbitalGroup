...
begin
  ...
  zamena:=false;
  //Прараметры ОГ:
  klv:=1;  //Количество витков
  vremya:= 0;
  k1:=1;
  k2:=1;
  k3:=1;
  //Параметры орбиты КА № 1 - акцептора

  fokal1:=R_ekv/1000+1000;  //фокальный параметр
  naklon1:=45;               //наклонение
  vosxod1:=0;               //прямое восхожд восх. узла, град.
  peregei1:=0;                                  //положение перигея  град.
  eks1:=0;                                       //Эксцентриситет
  Form1.RasschetOrbit_KEO(fokal1,eks1,naklon1,vosxod1,peregei1, shag,orbita1);

  //Параметры орбиты КА № 2 - донора
  fokal2:=R_ekv/1000 +4000;    //фокальный параметр
  naklon2:=90;               //наклонение
  vosxod2:=0;               //прямое восхожд восх. узла, град.
  peregei2:=0;                                  //положение перигея  град.
  eks2:=0;                                       //Эксцентриситет
...
function Tform1.Grad(rad:extended): Extended; //  радианы в градусы
 begin
  Result:=rad*180/PI;
 end;

function Tform1.Rad(grad:extended): Extended; //  градусы в радианы
 begin
  Result:=grad*PI/180;
 end;
...
procedure TForm1.RasschetOrbit_KEO( //Рассчёт орбиты  по КЭО
                            p, e, i, OmB, OmM, //КЭО:p- фокальный параметр, км.
                                               //    e- эксцентиситет,
                                               //    i- наклонение,   град.
                                               //    OmB - прямое восхожд восх. узла, град.
                                               //    OmM - положение перигея  град.
                            h: Extended;       // h- шаг изменения ист. аном., град.
                            out KOORD:Tmass2D  // Массив выходных данных (Координат)
                            );
 var
            // Счётчик
  {Hh,}  k :integer;  // Переменная для записи колличиства вычислений
  teta,         //Переменная для записи значения истинной аномалии.
  X, Y, Z, Vx, Vy, Vz,fff,ggg, vu: extended ;  //Переменные для записи координат
 begin
  // h:=0.05;
  Hh:= trunc(360*klv/h); //Вычисляем колличество шагав
  teta:=0;
  ...
  begin
   try
    ...
    messagedlg('Совокупность элементов исходной орбиты не является кеплеровской орбитой',
                                                          mtWarning, [mbOk],0);
    abort;
   end;
  end;
...
begin
   //Прараметры ОГ:
 // klv:=StrToInt(form3.Edit7.Text);  //Количество витков
  vremya:= 0;
  k1:=1;
  k2:=1;
  k3:=1;
  //Параметры орбиты КА-акцептора
  fokal1:=(R_ekv/1000 +StrToFloat(form3.Edit4.text));  //фокальный параметр
  naklon1:=StrToFloat(form3.Edit5.text);               //наклонение
  vosxod1:=StrToFloat(form3.Edit6.text);               //прямое восхожд восх. узла, град.
  peregei1:=StrToFloat(form3.Edit1.text);                                  //положение перигея  град.

  ekscentr1:=form3.Edit2.text;                    //Эксцентриситет
  ...
  //Параметры орбиты КА № 2  - донора
  fokal2:=R_ekv/1000 +StrToFloat(form3.Edit11.text);    //фокальный параметр
  naklon2:=StrToFloat(form3.Edit12.text);               //наклонение
  vosxod2:=StrToFloat(form3.Edit13.text);               //прямое восхожд восх. узла, град.
  peregei2:=StrToFloat(form3.Edit14.text);                                 //положение перигея  град.

  ekscentr2:=form3.Edit15.text;                    //Эксцентриситет
  if ekscentr2[2]='.' then  ekscentr2[2]:=',' ;
  eks2:=StrToFloat(ekscentr2);                                     //Эксцентриситет
...
begin
  ...
  ini.WriteBool('chekbox5', 'checked', form2.CheckBox5.Checked );
...
begin
 Ini:=TiniFile.Create(extractfilepath(paramstr(0))+'Parametry.ini');
  ...
  form2.CheckBox2.Checked:=ini.ReadBool('chekbox2', 'checked', form2.CheckBox2.Checked);
  form2.CheckBox3.Checked:=ini.ReadBool('chekbox3', 'checked', form2.CheckBox3.Checked);
  form2.CheckBox5.Checked:=ini.ReadBool('chekbox5', 'checked', form2.CheckBox5.Checked);
